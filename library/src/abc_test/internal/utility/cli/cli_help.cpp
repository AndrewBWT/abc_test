...
{
    using namespace std;
    _a_cli_output.set_terminate_early(true);
    if (_a_args.size() != 0)
    {
    ...
    {
        _a_cli_output.add_table_output(_a_cli.make_help_output());
    }
}
        ...
        if (_l_config_file.is_open())
        {
            ...
            while (getline(_l_config_file, _l_line))
            {
                ...
                else
                {
                    ...
                    _a_cli.process_config_line(_l_field_name, _l_strs);
                }
            }
        }
        else
    ...
    ) const noexcept
{
    ...
    else
    {
        ...
        abc::utility::parser::parse_result_t<rep_file_index_t> _l_rfli{ abc::utility::parser::parse<rep_file_index_t>(_a_args[1]) };
        abc::utility::parser::parse_result_t<rep_write_data_type_t> _l_rf{ abc::utility::parser::parse<rep_write_data_type_t>(_a_args[2]) };
        get<1>(_l_rep_data) = _l_rfli.value();
        get<2>(_l_rep_data) = _l_rf.value();
        _a_cli.process_repetition_data(_l_rep_data, _a_cli_output);
        if (not _a_cli_output.can_continue())
        {
            return;
        }
    }
