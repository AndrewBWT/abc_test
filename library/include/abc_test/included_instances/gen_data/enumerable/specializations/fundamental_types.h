...
struct enumerable_t<T>
{
		...
		)
	{
		...
		{
			const T _l_max_times_called{ static_cast<T>((_a_max_value.value() - _l_current_idx) / _m_difference) };
		...
		)
	{
		...
		if (_a_max_value.has_value())
		{
			const T _l_max_times_called{ static_cast<T>((_l_current_idx - _a_max_value.value()) / _m_difference) };
			if (_a_times_called > _l_max_times_called)
			{
				if (_l_max_times_called == 0)
				{
...
{
	if (_a_n_times_to_increment > 0 && _a_element == false && _a_max_value == true)
	{
		_a_element = true;
...
{
	if (_a_n_times_to_increment > 0 && _a_element == true && _a_max_value == false)
	{
		_a_element = false;
		_a_n_times_to_increment--;
		return true;
