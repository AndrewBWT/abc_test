...
_TEST_CASE(
    abc::test_case_t(
        {.name = "Check parsing of a correct repetition configuration",
...
_TEST_CASE(
    abc::test_case_t(
        {.name = "Check parsing of an incorrect repetition configuration",
         .path = "abc_test_test::component_tests::repetition_config",
         .threads_required = 1}
    )
)
{
    ...
    _l_mds.insert(u8"func", _l_trie_1);
    _l_mds.insert(u8"func", _l_trie_1);
    auto _l_str = print_map_of_unique_ids_to_tdg_collection_stack_tries(_l_mds);
    u8string _l_str2 = fmt::format(u8"{0}:{0}", _l_str);
    vector<pair<u8string, u8string>> _l_strs = {
        {u8"hi:",      u8"Checking incorrect number of colons"                  },
        {u8"hi",       u8"Checking incorrect number of colons"                  },
        {u8"hi:hi:hi", u8"Checking incorrect number of colons"                  },
        {u8":hi",      u8"Checking empty test name"                             },
        {u8"hi:",      u8"Checking empty RC info"                               },
        {u8"hi:12",    u8"Checking incorrect hex for test name"                 },
        {u8"12:hi",    u8"Checking incorrect hex for RC"                        },
        {_l_str2,       u8"Checking map already contains function with same namd"},
    };
    for (auto&& [_l_str, _l_identifier] :
         iterate_over<pair<u8string, u8string>>(_l_strs))
    {
        vector<u8string> _l_args = {
            u8"--write_data_to_files",
            u8"false",
        ...
        _l_args.push_back(_l_str);
        auto _l_result
            = abc_test::utility::abc_test_tests_command_line_test_runner(
                1, _l_args, {}
            );
        _CHECK(annotate(
            _EXPR(_l_result.simple_text_reporter.errors().size() > 0),
            _l_identifier
        ));
    }
}
