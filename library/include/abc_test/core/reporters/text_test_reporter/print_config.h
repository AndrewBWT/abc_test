...
public:
	...
	__constexpr
		const std::string_view
		seed_to_use_to_re_run_test_str(
		) const noexcept;
	__constexpr
		std::string
	...
	__constexpr
		const std::string_view
		seed_to_use_to_re_run_test_in_hex_str(
		) const noexcept;
	__constexpr
		std::string
...
{
	return slight_highlight(_a_seed == "" ? "<no seed used>" : _a_seed);
}
__constexpr_imp
const std::string_view
print_config_t::seed_used_in_hex_str(
) const noexcept
{
...
) const noexcept
{
	return slight_highlight(
		_a_seed == "" ? "<no seed used>" : quote(_a_seed));
}
__constexpr_imp
const std::string_view
print_config_t::seed_to_use_to_re_run_test_str(
) const noexcept
{
...
__constexpr_imp
const std::string_view
print_config_t::seed_to_use_to_re_run_test_in_hex_str(
) const noexcept
{
...
__constexpr_imp
const std::string_view
print_config_t::test_warnings_recieved_str(
) const noexcept
{
	return "Warnings";
}
...
{
	return highlight_pass_or_fail(_a_warnings == 0, fmt::format("{0}",_a_warnings));
}
__constexpr_imp
const std::string_view
print_config_t::assertions_recieved_str(
) const noexcept
{
...
__constexpr_imp
const std::string_view
print_config_t::assertions_passed_str(
) const noexcept
{
...
{
	return highlight_pass_or_fail(_a_assertions > 0, fmt::format("{0}", _a_assertions));
}
__constexpr_imp
const std::string_view
print_config_t::assertions_failed_str(
) const noexcept
{
...
__constexpr_imp
const std::string_view
print_config_t::compat_assertion_data_str(
) const noexcept
{
...
{
	return (_a_failed == 0) ?
		highlight_pass(_a_total_recieved == 0 ?
			fmt::format("No assertions ran.") :
			fmt::format("{0} / {1} assertions passed. All assertions passed.",
			_a_passed, _a_total_recieved)) :
		highlight_fail(fmt::format("{0} / {1} assertions passed. {2} {3} failed.",
			_a_passed, _a_total_recieved,(_a_passed == 0) ? "All" :
		fmt::format("{0}",_a_failed),
			(_a_failed == 1) ? "assertion" : "assertions"));
}
__constexpr_imp
const std::string_view
print_config_t::termination_status_str(
) const noexcept
{
...
__constexpr_imp
std::string
print_config_t::termination_status(
	const enum_termination_type_t _a_termination_type
) const
{
	using enum enum_termination_type_t;
	switch (_a_termination_type)
	{
	case NO_TERMINATION:
...
{
	return highlight_pass_or_fail(_a_passed,
		_a_passed ? "Yes" : "No");
}
__constexpr_imp
const std::string_view
print_config_t::name_str(
) const noexcept
{
	return "Test name";
}
...
{
	return highlight(quote(_a_name.value()));
}
__constexpr_imp
const std::string_view
print_config_t::str_status_str(
) const noexcept
{
	return "Test result";
}
...
__constexpr_imp
enum_print_pair_collection_t< enum_after_execution_test_report_fields_t>
print_config_t::after_execution_test_report_fields(
) const noexcept
{
...
{
	return _m_manual_assertion_fields;
}
__constexpr_imp
enum_print_pair_collection_t< combined_enum_generic_assertion_fields_t>
print_config_t::static_assertion_fields(
) const noexcept
{
...
{
	return slight_highlight(_a_str.has_value() ?
		quote(_a_str.value()) :
		"<no message>");
}
__constexpr_imp
