...
#include "abc_test/core/matchers/comparison/comparison_enum.h"
...
__constexpr matcher_t
    make_matcher(T1&& _a_left_arg, T2&& _a_right_arg) noexcept;
} // namespace
...
{
    using namespace std;
    return matcher(new comparison_matcher_t<T1, T2, Cmp_Enum>(
        forward<T1>(_a_left_arg), forward<T2>(_a_right_arg)
    ));
}
} // namespace
