...
template <typename T>
inline void
    fuzzy_tests_for_static_make_rng_functions()
{
    using namespace abc;
    using namespace abc::data_gen;
    using namespace std;
...
)
{
    ...
    RUN(_l_mdg, fuzzy_tests_for_static_make_rng_functions<simple_rng_t>());
    RUN(_l_mdg,
        fuzzy_tests_for_static_make_rng_functions<inner_rng_mt19937_64_t>());
}
...
template <typename T>
inline void
    property_test_for_progress_and_operator(
...
)
{
    ...
    using namespace utility;
    manual_data_generator_t _l_mdg;
    RUN(_l_mdg, property_test_for_progress_and_operator<simple_rng_t>(100'000));
    RUN(_l_mdg,
        property_test_for_progress_and_operator<inner_rng_mt19937_64_t>(100'000)
    );
}
    ...
    )
{
    ...
    auto _l_rnd_generator{default_random_generator<unit_test_1>(
        default_random_generator<variant<pair<unsigned int, size_t>, seed_t>>(
            default_random_generator<pair<unsigned int, size_t>>(
                default_random_generator<unsigned int>(),
                // Limit constructor to take first 100k values to create rng.
                default_random_generator<size_t>(0, 100'000)
            ),
            default_random_generator<seed_t>()
        ),
...
)
{
    ...
    using namespace utility;
    manual_data_generator_t _l_mdg;
    RUN(_l_mdg, (fuzzy_rng_tests<simple_rng_t>()));
    RUN(_l_mdg, fuzzy_rng_tests<inner_rng_mt19937_64_t>(100'000));
}