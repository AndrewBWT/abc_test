...
__no_constexpr void
    skip_chars_and_strings_until_found(
        const std::u8string_view _a_str,
        std::size_t&             _a_current_pos_ref,
        const std::initializer_list<std::pair<char8_t, char8_t>>&
                          _a_ignore_mode,
...
{
    using namespace std;
    locate_relevant_character(_a_str, u8",]", _a_current_pos, _a_error_string);
    ...
    ) noexcept
{
    ...
    while (_l_depth > 0)
    {
        ...
        {
            size_t _l_next_relevant_char
                = _a_str.find(_l_ignore_char.value(), _a_current_pos_ref);
        ...
        {
            size_t _l_next_relevant_char
                = _a_str.find_first_of(_l_to_find, _a_current_pos_ref);
            if (_l_next_relevant_char == string::npos)
            {
            ...
            else if (_l_char == _a_decrement_counter)
            {
                ...
                {
                    _a_current_pos_ref = _l_next_relevant_char;
                    int i = 58;
                }
            }
            else
