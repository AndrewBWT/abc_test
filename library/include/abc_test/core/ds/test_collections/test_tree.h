...
public:
	/*!
	 * @brief Default constructor. Contstructs an empty tree.
	 */
	__constexpr
		test_tree_t(
		) noexcept = default;
	 ...
	 * @param _a_options Test options, used to perform checks on the element to be added.
	 * @return An optional string; if it has an element, then an error occoured. If not, 
	 * no error occoured. 
	 */
	__no_constexpr
	...
	/*!
	 * @brief Gets the caller the begin iterator of the tree.
	...
	/*!
	 * @brief Gets the caller the end iterator of the tree.
	 * @return The end iterator of the tree.
	 */
	__no_constexpr_or_inline
		test_tree_iterator_t
		end(
...
) noexcept
{
	...
	const test_path_hierarchy_t& _l_test_hierarchy{ _a_test.get().test_path_hierarchy()};
		...
		if (_m_nodes_tests.size() > 0)
		{
			auto _l_x = upper_bound(_m_nodes_tests.begin(), _m_nodes_tests.end(), _a_test,
				[](const node_t& _a_left, const node_t& _a_right) {
					return _a_left.get().registered_test_data()._m_user_data.name <
						_a_right.get().registered_test_data()._m_user_data.name;
				}
			);
