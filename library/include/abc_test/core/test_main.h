...
#include "abc_test/core/reporters/text_error_reporter.h"
...
_BEGIN_ABC_NS
	/*!
	* The main runner. This object holds the majority of logic concerning the ordering of things
	* within the system.
	*/
	struct test_main_t
	{
	public:
		...
		* and it is up to the user to deallocate memory (if applicable). 
		*/
		__constexpr
			void
			add_test_list(
		...
		/*!
		* Adds a test list using a reference. test_main does not take ownership of any underlyilng pointer,
		* or copy the data. The user must ensure that the reference stays alive for the lifetime of the 
		* test_main object.
		*/
		__constexpr
			void
		...
		*/
		...
		/*!
		* Adds a test_reporter_t using a reference. test_main does not take ownership of any underlyilng pointer,
		* or copy the data. The user must ensure that the reference stays alive for the lifetime of the
		* test_main object.
		*/
		__constexpr
		...
		*/
		...
		/*!
		* Adds a error_reporter_t using a reference. test_main does not take ownership of any underlyilng pointer,
		* or copy the data. The user must ensure that the reference stays alive for the lifetime of the
		* test_main object.
		*/
		__constexpr
...
_BEGIN_ABC_NS
		...
		) noexcept
	{
		...
		_m_thread_pool = (_m_options._m_threads >
			thread::hardware_concurrency() ||
			_m_options._m_threads == 0) ?
			thread::hardware_concurrency() :
			_m_options._m_threads;
		_LIBRARY_LOG(MAIN_INFO, fmt::format(
		...
		test_options_t& _l_global_test_options{ global::setup_global_variables(_m_options) };
		error_reporter_controller_t& _l_erc{ global::get_global_error_reporter_controller() };
		test_reporter_controller_t& _l_global_trc{ global::get_global_test_reporter_controller() };
		...
		while (_l_ftc.remaining_elements() > 0 && _l_erc.should_exit() == false)
		{
			...
			{
				_LIBRARY_LOG(MAIN_INFO, 
					fmt::format("Waiting as _m_current_thread_pool ({0}) < _l_thread_next_size ({1}).",
					_m_current_thread_pool, _l_next_thread_size));
				//Wait until we do have the resourses.
				unique_lock _l_thread_pool_lock(_m_thread_pool_mutex);
				_m_cv.wait(_l_thread_pool_lock, 
			...
			if (_l_erc.should_exit() == false)
			{
				...
				_m_threads[_l_thread_idx] = jthread(&test_main_t::run_individual_thread, this, _l_test, _l_thread_idx);
			}
			else
			{
				_LIBRARY_LOG(MAIN_INFO, "_l_erc == false, we are terminating this loop.");
			}
		}
		...
		if (_l_erc.should_exit())
		{
			...
			_l_erc.report_information(fmt::format(
				"One (or more) of thre tests reported a catastrophic error which could not be recovered from. "
		...
		) noexcept
	{
		...
		if (_m_test_reporters.size() == 0)
		{
			...
			{
				cout <<
					"WARNING: No test reporters have been registered to the test_main_t object. "
					"The test suite will be ran, however no results of any kind will be reported. "
					"If this circumstance is encountered again, and you wish for a simple console test reporter "
			...
			else
			{
				cout <<
					"WARNING: No test reporters have been registered to the test_main_t object. "
					"A basic console output test reporter will be added, as we assume this is in error. "
					"If you wish to run the test_main_t object without any test reporters, in the associated "
					"test_options_t object, set \"_m_allow_empty_test_reporters_to_be_used\" to true." <<
					endl;
				_m_test_reporters.push_back(shared_ptr<test_reporter_t>(new text_test_reporter_t()));
			}
		}
		if (_m_error_reporters.size() == 0)
		{
			...
			{
				cout <<
					"WARNING: No error reporters have been registered to the test_main_t object. "
					"The test suite will be ran, however no internal logic errors will be reported. "
					"If this circumstance is encountered again, and you wish for a simple console error reporter "
					"to be added, in the associated test_options_t object, set "
			...
			else
			{
				cout <<
					"WARNING: No error reporters have been registered to the test_main_t object. "
					"A basic console output error reporter will be added, as we assume this is in error. "
					"If you wish to run the test_main_t object without any error reporters, in the associated "
					"test_options_t object, set \"_m_allow_empty_error_reporters_to_be_used\" to true." <<
					endl;
				_m_error_reporters.push_back(shared_ptr<error_reporter_t>(new text_error_reporter_t()));
			}
		}
	}
_END_ABC_NS