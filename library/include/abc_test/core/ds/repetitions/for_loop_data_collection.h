...
public:
     ...
     */
    __constexpr
    for_loop_data_collection_t() noexcept;
    /*!
     * Function adds an element to the internal stack.
     *
     * Arguments are the on_error_function for the current for loop value,
     * the current generation_collection index, and a pointer to the repetition
     * data.
     *
     * We use the pointer as we cannot have a pointer/reference to the gen_data
     * object, as it would have a template parameter we cannot reasonably be
     * expected to know. This way, we can retain enough information to encode
     * any repetition data.
     */
     ...
     */
    __constexpr ds::repetition_data_sequence_t
                inform_test_generators_of_failure() const noexcept;
    /*!
     * Builds a repetition sequence from the data contained within.
     */
    __constexpr repetition_data_sequence_t
        repetition_data_sequence() const noexcept;
    /*!
     * Gets a reference to the underlying for_loop_data_t vector.
     */
    const __constexpr std::vector<for_loop_data_t>&
                      for_loop_data_collection() const noexcept;
    /*!
     * Gets the current for loop index. This value represents the next value any
     * new for loop should take in the current stack of for loops; making a new
     * for loop will reset this back to zero, and unwinding the current for loop
     * will take the previous index and increment it.
     */
...
template <>
struct fmt::formatter<abc::ds::for_loop_data_collection_t>
...
_BEGIN_ABC_DS_NS
__constexpr_imp
    for_loop_data_collection_t::for_loop_data_collection_t() noexcept
...
{
    _m_for_loop_data_collection.push_back(for_loop_data_t(
        _m_current_for_loop_index,
        _a_on_error_function,
    ...
    for_loop_data_collection_t::decrement()
{
    ...
    {
        _m_current_for_loop_index
            = (_m_for_loop_data_collection.back().for_loop_index()) + 1;
        _m_for_loop_data_collection.pop_back();
    }
    else
    {
        throw test_library_exception_t(fmt::format(
            "decrement called when _m_for_loop_data_collection size == 0"
        ));
    }
}
    ...
    )
{
    ...
    if (_m_for_loop_data_collection.size() > 0)
    {
        ...
        //	_a_on_error_function, _a_generation_collection_index,
        //_a_mode,_a_additional_data);
        _m_current_for_loop_index = 0;
    }
    else
    {
        throw test_library_exception_t(fmt::format(
            "update called when _m_for_loop_data_collection size == 0"
        ));
    }
}

__constexpr_imp ds::repetition_data_sequence_t
...
__constexpr_imp repetition_data_sequence_t
    for_loop_data_collection_t::repetition_data_sequence() const noexcept
{
    repetition_data_sequence_t _l_rds{};
    for (const for_loop_data_t& _l_fld : _m_for_loop_data_collection)
    {
        _l_rds.push_back(repetition_data_t{
            _l_fld.for_loop_index(),
            for_loop_iteration_data_t{
                                      _l_fld.generation_collection_index(), _l_fld.repetition_data()
            }
        });
    }
    return _l_rds;
...
const __constexpr_imp std::vector<for_loop_data_t>&
    for_loop_data_collection_t::for_loop_data_collection() const noexcept
{
    return _m_for_loop_data_collection;
}

__constexpr_imp std::size_t
    for_loop_data_collection_t::current_for_loop_index() const noexcept
{
