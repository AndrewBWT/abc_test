...
public:
    ...
    __constexpr bool
        subclass_has_current_element() const noexcept override;
    //__constexpr
    //	std::string
    ...
    __constexpr_imp void virtual subclass_set_data_using_mode_and_repetition_data(
    ...
    ) override final
    {
        ...
        _m_has_current_element
            = _m_enumerable.move_forward(this->_m_element, _l_data);
    }

    __constexpr enumerate_data_id_type_t
    ...
    __constexpr bool
        subclass_generate_next() override;
private:
...
__constexpr data_generator_collection_t<T,true>
            enumerate_data(const enumerate_t<T>& _a_enumerate) noexcept;
template <typename T>
__constexpr data_generator_collection_t<T,true>
...
template <typename T>
__constexpr data_generator_collection_t<T,true>
    randomly_probe_enumerated_data(const enumerate_t<T>& _a_enumerate) noexcept;

template <typename T>
requires max_value_c<T> && min_value_c<T>
__constexpr_imp enumerate_t<T>
                all_values(
...
__constexpr_imp const std::string_view
                      enumerate_data_file_type() noexcept
{
    return "ham";
   // return global::get_global_test_options()._m_enumerate_data_file_type;
}
...
{
    return (_m_forward) ? _m_enumerable->increment(
                              _a_element, _a_times_called, _m_end_value
                          )
                        : _m_enumerable->decrement(
                              _a_element, _a_times_called, _m_end_value
                          );
...
{
    return (_m_forward) ? _m_enumerable->decrement(
                              _a_element, _a_times_called, _m_start_value
                          )
                        : _m_enumerable->increment(
                              _a_element, _a_times_called, _m_start_value
                          );
}
...
__constexpr_imp
    ...
    : data_generator_with_file_support_and_element_storage_t<
          ...
          {},"hello",
          //_a_test_options->_m_enumerate_data_file_type,
          _a_enumerate_opt_data_file_name,
          _a_enumerate.min(),
          _a_test_options
      )
...
template <typename T>
__constexpr_imp bool
    enumerate_data_t<T>::subclass_has_current_element() const noexcept
{
...
template <typename T>
__constexpr_imp bool
    enumerate_data_t<T>::are_failed_values_written_to_files() const noexcept
{
    return true;
}
    ...
    enumerate_data_t<T>::subclass_generate_next()
{
    if (_m_has_current_element)
    {
        ...
        _m_has_current_element
            = _m_enumerable.next_element(this->_m_element, _l_adx);
    }
    return _m_has_current_element;
}
                ...
                ) noexcept
{
    ...
    return unary_collection<T>(new enumerate_data_t<T>(
        _a_enumerate, opt_file_rw_info_t<T>{}, opt_file_name_t{}
    ));
}
                ...
                ) noexcept
{
    using namespace utility::io;
    return unary_collection<T>(new enumerate_data_t<T>(
        _a_enumerate, opt_file_rw_info_t<T>{_a_file_data}, opt_file_name_t{}
    ));
}
                ...
                ) noexcept
{
    using namespace utility::io;
    return unary_collection<T>(new enumerate_data_t<T>(
        _a_enumerate, opt_file_rw_info_t<T>{}, opt_file_name_t{_a_file_name}
    ));
}
                ...
                ) noexcept
{
    ...
    return unary_collection<T>(new enumerate_data_t<T>(
        _a_enumerate,
        opt_file_rw_info_t<T>{_a_file_data},
        opt_file_name_t{_a_file_name}
    ));
}
...
{
    return enumerate_t<T>(
        _a_enumerable, _a_start_value, _a_elements, _a_forward_direction
    );
}
...
{
    return from_m_enumerate_k_elements(
        _a_start_value, _a_elements, true, _a_enumerable
    );
}
...
{
    return from_m_enumerate_k_elements(
        _a_start_value, _a_elements, false, _a_enumerable
    );
}
