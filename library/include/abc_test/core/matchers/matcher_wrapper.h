...
#include "abc_test/core/matchers/logic/logic_matcher.h"
...
__no_constexpr_imp matcher_t
    matcher(matcher_base_t* _a_generic_matcher_ptr) noexcept;
    ...
    ) noexcept
{
    ...
    *this = matcher(new logic_matcher_t<OR>(
    ...
    ) noexcept
{
    ...
    *this = matcher(new logic_matcher_t<AND>(
        this->internal_matcher(), std::shared_ptr<matcher_base_t>()
    ));
    _m_matcher_internal->add_source_info(_a_single_source);
    return _l_result;
...
{
    return matcher(new logic_matcher_t<AND>(
...
{
    return matcher(new logic_matcher_t<OR>(
        this->internal_matcher(), _a_matcher.internal_matcher()
    ));
}
