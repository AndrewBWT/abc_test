...
template <typename bool Is_Root>
struct fmt::formatter<abc::ds::repetition_tree_node_t<Is_Root>>
...
template <typename bool Is_Root>
__constexpr_imp std::string
    repetition_tree_node_t<Is_Root>::print_repetition_tree() const noexcept
{
    using namespace std;
    using namespace utility::str;
    string _l_children_str{_c_l_square_bracket};
    for (size_t                     _l_idx{0};
         const rep_tree_children_t& _l_for_loop_at_idx : _m_children)
    {
        _l_children_str.append(_c_l_square_bracket);
        for (size_t                  _l_jdx{0};
             const rep_tree_child_t& _l_child : _l_for_loop_at_idx)
        {
            _l_children_str.append(_l_child->print_repetition_tree());
...
template <typename bool Is_Root>
__constexpr_imp opt_repetition_data_t
    ...
    ) const noexcept
{
    ...
    else if (_a_idx + 1 >= _a_rds.size())
    {
        ...
        {
            const rep_tree_children_t::const_iterator _l_itt{
                _l_opt_itt.value() + 1
            };
    ...
    ) const noexcept
{
    ...
    if (_a_idx >= _a_rds.size())
    {
        ...
        {
            return repetition_data_t{0, _m_children[0][0]->_m_for_loop_data};
        }
        else
        {
    ...
    else
    {
            ...
            _l_opt_itt.has_value())
        {
            return (*_l_opt_itt.value())
                .get()
                ->find_next_for_loop(_a_rds, _a_idx + 1);
        }
        else
        {
            return opt_repetition_data_t{};
    ...
    ) noexcept
{
    ...
    if (_a_idx < _a_rds.size())
    {
        ...
        vector<shared_ptr<repetition_tree_node_t<false>>>& _l_rtn{
        ...
        shared_ptr<repetition_tree_node_t<false>> _l_new_child{
            shared_ptr<repetition_tree_node_t<false>>(
                new repetition_tree_node_t<false>(_l_rd.for_loop_iteration_data)
            )
        };
        ...
        vector<shared_ptr<repetition_tree_node_t<false>>>::iterator _l_itt
...
__constexpr_imp opt_itt_t
    repetition_tree_node_t<Is_Root>::get_itt_to_rep_tree_node(
        const repetition_data_t& _a_rd
    ) const noexcept
{
    using namespace std;
    ...
    const rep_tree_children_t& _l_vect{_m_children[_a_rd.for_loop_index]};
    rep_tree_children_t::const_iterator _l_lower{_l_vect.begin()};
    rep_tree_children_t::const_iterator _l_upper{_l_vect.end()};
    const for_loop_iteration_data_t& _l_rd_fid{_a_rd.for_loop_iteration_data};
    do
    {
        ...
        {
            rep_tree_children_t::const_iterator _l_itt_to_check{
                _l_lower + ((_l_upper - _l_lower) / 2)
            };
            const for_loop_iteration_data_t& _l_flid{
...
template <bool Is_Root>
__constexpr_imp parse_repetition_tree_result_t<Is_Root>
                ...
                ) noexcept
{
    ...
    else
    {
        ...
        for (const vector<string>& _l_str : _l_strs)
        {
            ...
            {
                const expected<repetition_tree_node_t<false>, string> _l_op{
                    parse_compressed_repetition_tree_node<false>(_l_st)
                };
                if (_l_op.has_value())
                {
                    _l_kids.push_back(shared_ptr<repetition_tree_node_t<false>>(
