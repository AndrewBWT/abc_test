...
_BEGIN_ABC_NS
	...
	template<
		typename T
	>
	struct enumerable_t<std::vector<T>>
	{
	private:
		...
		) const noexcept
	{
			...
			)
		{
			...
			while (_a_n_times_to_increment > 0)
			{
				if (_a_array.size() == 0)
				{
					either_increment_vector_or_create_next_largest_vector(_a_array, _a_n_times_to_increment);
					//_a_array.resize(1);
					//_a_n_times_to_increment--;
						...
						for (size_t _l_idx{ _a_array.size() - 1}; _l_idx > 0; --_l_idx)
						{
							...
							else
							{
								...
								if (_l_idx == 1)
								{
									...
									for (size_t _l_idx{ 0 }; _l_idx < _a_array.size(); ++_l_idx)
									{
