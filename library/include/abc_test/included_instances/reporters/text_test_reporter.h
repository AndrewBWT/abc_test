...
__no_constexpr_imp
		...
		) const
	{
		...
		//const size_t _l_line_len{ _m_print_config.line_len};
		//const string _l_line_break{fmt::format("{0}\n",string(
	//		_l_line_len, _m_print_config.line_len_char))};
		...
		const vector<vector<string>> _l_strs = get_all_data<false>(_m_print_config.after_execution_test_report_fields(),
			_a_itd, _m_print_config, test_report_list_formatter());
		...
		{
			const unexpected_report_t<true>& _l_unexpected_termination{ *_a_itd.unexpected_termination() };
			const vector<string> _l_strs = process_termination(_l_unexpected_termination);
