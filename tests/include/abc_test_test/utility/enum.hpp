...
enum class enum_with_enum_list_2 : uint32_t
{
    one
};
...
using type_list_t
    = abc::utility::type_list<enum_with_enum_list_1, enum_with_enum_list_2>;
} // namespace
...
_TEST_CASE(
    abc::test_case_t(
        {.name             = "Tests for enumerate_enum_helper_t::min and max",
         .path             = "abc_test_test::utility::enum",
         .threads_required = 1}
    )
)
{
    ...
    auto _l_test_func = [&]<typename Enum>()
    {
        auto _l_type_name{typeid(Enum).name()};
        _BEGIN_MULTI_ELEMENT_BBA(
            _l_unit_tests, "Unit testing less_than and equal"
        );
        using test_data_t = std::tuple<Enum, Enum>;
        enumerate_enum_helper_t<Enum> _l_helper(get_enum_list<Enum>());
        for (const auto& _l_data : read_data_from_file<test_data_t>(
                 fmt::format("unit_tests_{0}", _l_type_name)
             ))
        {
            auto& [_l_min, _l_max]{_l_data};
...
)
{
    ...
    auto _l_test_func = [&]<typename Enum>()
    {
        ...
        auto _l_fuzzy_test
            = [&]<typename F>(const string_view _a_str, F _a_func)
        {
            for (const auto& _l_data :
                 generate_data_randomly<fuzzy_test_data_t>())
            {
                _TVLOG(_l_data);
        ...
        _l_fuzzy_test("increment", _l_increment);
        _l_fuzzy_test("decrement", _l_decrement);
        _END_BBA_CHECK(_l_fuzzy_tests);
        ...
        auto _l_property_test_func = [&]<typename F>(F _a_func)
        {
            for (auto& _l_data : enumerate_data<property_test_data_1>())
            {
                _TVLOG(_l_data);
                auto& [_l_lower, _l_upper]{_l_data};
                     ...
                     enumerate_data<Enum>(from_m_to_n(_l_lower, _l_upper)))
                {
                    ...
                    for (auto& _l_size : generate_data_randomly<size_t>()
                                             & iterate_over<size_t>(
...
)
{
    ...
    auto _l_test_func = [&]<typename Enum>()
    {
        ...
        using unit_test_type_print
            = std::tuple<Enum, u8string, optional<enum_helper_string_type_e>>;
        for (auto& _l_data : read_data_from_file<unit_test_type_print>(
                 fmt::format("unit_test_print_{0}", _l_type_name)
             ))
        {
            _TVLOG(_l_data);
            auto& [_l_enum, _l_expected, _l_str_type]{_l_data};
        ...
        {
            _TVLOG(_l_data);
            auto& [_l_expected_enum, _l_expected_elements_remaining, _l_str, _l_str_type]{
                _l_data
            };
            ...
            _l_unit_tests += _BLOCK_CHECK(
                ...
                && _EXPR(
                    _l_pi.elements_left() == _l_expected_elements_remaining
                )
            );
        }
        _END_BBA_CHECK(_l_unit_tests);
        ...
        for (auto& _l_data : generate_data_randomly<fuzzy_test_type_parse>())
        {
            ...
            case 0:
                _l_str_input
                    = _l_helper.print(get<0>(_l_input), _l_str_type.value());
                break;
            case 1:
        ...
        for (auto& _l_data : enumerate_data<property_test_type>())
        {
            ...
            if (_l_str_type.has_value())
            {
                ...
                _l_enum_result = _l_helper.parse_enum(
                    _l_pi,
                    _l_str_type.value()
                );
            }
        ...
        using error_test_type_print
            = std::tuple<Enum, optional<enum_helper_string_type_e>, u8string>;
        for (auto& _l_data : read_data_from_file<error_test_type_print>(
                 fmt::format("error_test_print_{0}", _l_type_name)
             ))
        {
            _TVLOG(_l_data);
            auto& [_l_enum, _l_str_type, _l_expected_error]{_l_data};
            matcher_t _l_matcher;
            __BEGIN_MAKE_MATCHER_TO_CHECK_EXCEPTION_TYPE_AND_MSG(_l_matcher);
            if (_l_str_type.has_value())
            {
                do_not_optimise(_l_helper.print(_l_enum, _l_str_type.value()));
            }
            else
            {
                do_not_optimise(_l_helper.print(_l_enum));
            }
            __END_MAKE_MATCHER_TO_CHECK_EXCEPTION_TYPE_AND_MSG(
        ...
        using error_test_type_parse = std::
            tuple<u8string, optional<enum_helper_string_type_e>, u8string>;
        for (auto& _l_data : read_data_from_file<error_test_type_parse>(
                 fmt::format("error_test_parse_{0}", _l_type_name)
             ))
        {
            _TVLOG(_l_data);
            auto& [_l_str, _l_str_type, _l_expected_error]{_l_data};
            matcher_t _l_matcher;
            __BEGIN_MAKE_MATCHER_TO_CHECK_EXCEPTION_TYPE_AND_MSG(_l_matcher);
            parser::parser_input_t _l_pi(_l_str);
            if (_l_str_type.has_value())
            {
                do_not_optimise(_l_helper.parse_enum(_l_pi, _l_str_type.value())
                );
            }
            else
            {
                do_not_optimise(_l_helper.parse_enum(_l_pi));
            }
            __END_MAKE_MATCHER_TO_CHECK_EXCEPTION_TYPE_AND_MSG(
                _l_matcher, abc::unreachable_exception_t, _l_expected_error
            );
