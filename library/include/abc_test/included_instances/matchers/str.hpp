        ...
        else
        {
            ...
            else
            {
                auto _l_add_reaosn_to_explanation
                    = [&]<typename TType, size_t Type_Index>(
                      ...
                      )
                {
                    _l_explanation_strs.push_back(fmt::format(
                        ...
                        u8"inhabiting "
                        u8"the {3} index in the ovearching type.",
                        positive_integer_to_placement(_a_expected_idx),
                        type_id<TType>(),
                        positive_integer_to_placement(Type_Index)
                    ));
                };
                _l_explanation_strs.push_back(fmt::format(
                ...
                _l_add_reaosn_to_explanation.operator()<T, 0>(1);
                _l_add_reaosn_to_explanation.operator()<U, 1>(2);
                _l_explanation_strs.push_back(fmt::format(
                    u8"When comparing the type indexes using {0}, "
                    u8"the matcher fails.",
                    cmp_str<Cmp>()
                ));
                return matcher_result_t(
                    false,
    ...
    expected_no_exception() noexcept
{
    ...
    return mk_matcher_using_result(matcher_result_t(
        ...
        matcher_result_infos_t(fmt::format(
            u8"No exception thrown, which was the expected behaviour"
        ))
    ));
}
