...
/*!
 * This function shows how to use static assertions; assertions which either
 * pass, fail or terminate the function.
 */
...
{
    /*!
     * Static assertions are the simplest types of assertions in the library.
     * They will alwways do one thing; either succeed, fail, or terminate. Below
     * we show these three basic assertions.
     *
     * To be very clear, all assertions in this library require a semi-colon
     * after them.
     *
     * The macros do not include semi-colons, as they are designed to be able to
...
{
    /*!
     * Like all assertions in this library, each static assertion can have an
     * assocaited annotation with it. The user can use these to describe what is
     * being tested.
     *
...
{
    /*!
     * All macros built around assertions in this library are, at a very basic
     * level, built around functions which return booleans. The booleans signify
     * whether the function passes or not.
     */
...
{
    /*!
     * These are what we believe the majority of users will use when writing
     * testing code.
     *
     * There are two core macros; _CHECK and _REQUIRE. _CHECK will fail with a
     * failed assertion, _REQUIRE will fail and terminate the function.
     *
     * Each takes a "matcher" as an argument. For now, assume that the
     * expression eq(a,b) is a matcher which compares the entities a and b.
     *
     *
     */
    using namespace abc;
     ...
     */
    _CHECK(annotate("Checking equality", eq(1, 2)));
...
)
{
    ...
    using namespace std;
    /*!
     * For this example, assume that the function ab::contains(R,i) checks if i
     * is in R.
     *
     * By using the results of a _CHECK, we can ensure that a computationally
     * expensive _CHECK is skipped if the first argument is true.
     *
     * This is done to simulate an OR statement.
     */
    vector<int> _l_vect(1'000);
    std::ranges::iota(_l_vect, 0);
    bool _l_result{false};
    if (_l_result |= _CHECK(eq(1, 2)))
    {
        /*!
         * This code is more computationally expensive to run, and should never
         * be hit.
         *
         * We use the function abc::matchers::contains from matchers/ranges to
         * run this code.
         *
         */
